...
<script>
  ...
  // go Active
  $('.btn-go-to-active').on('click', function(e){
    e.preventDefault();
    let ref = $(this).data("ref")
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            type:'POST',
            url:"{{ route('pages-user-go-to-active') }}",
            data:{ref:ref},
  ...
  //
  $('.btn-resend-activation-code').on('click', function(e){
    e.preventDefault();
    let ref = $(this).data("ref")
      Swal.fire({
        title: 'Are you sure?',
        text: "Resend Activation Code by Email?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Yes, Sure!',
        customClass: {
        ...
        if (result.value) {
          ...
          $.ajax({
            type:'POST',
            url:"{{ route('pages-user-resend-activation') }}",
            data:{ref:ref},
            success:function(data){
                if(data.success){
                  Swal.fire({
                    icon: 'success',
                    title: 'Active!',
                    text: 'Activation Code Has Been Resend',
                    customClass: {
                      confirmButton: 'btn btn-success'
                    }
                  });
                ...
                <td>
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu" style="">
                      ...
                      <a class="dropdown-item btn-resend-activation-code" href="#" data-ref="{{ $item->ref }}"><i class="ti ti-mail me-1 text-warning"></i> Resend Activation</a>
                      <a class="dropdown-item btn-go-to-active" href="#" data-ref="{{ $item->ref }}"><i class="ti ti-user-check me-1 text-success"></i> Active</a>
